...
{
	Factory<Shape> f;
	REQUIRE(f.registerType("circle", [] { return std::make_unique<Circle>(); }));
	REQUIRE(f.size() == 1);
...
TEST_CASE("factory creators capture construction context", "[core]")
{
	Factory<Shape> f;
	const int side = 7;
	f.registerType("square", [side] { return std::make_unique<Square>(side); }); // context in the closure
...
{
	Factory<Shape> f;
	REQUIRE(f.registerType("square", [] { return std::make_unique<Square>(1); }));		 // new -> true
	REQUIRE(f.registerType("circle", [] { return std::make_unique<Circle>(); }));		 // new -> true
	REQUIRE_FALSE(f.registerType("square", [] { return std::make_unique<Square>(2); })); // replace -> false
