...
namespace lain::io
{
	...
	// read(). Like read, `uri` is a core::Uri: a bare path or file:// writes a local file
	// (created, or truncated if it exists), and only that scheme is served today — remote/s3
	// dispatch behind this same function later. io moves bytes and never encodes: turning an
	// Image into format bytes is a codec's job (lain::io::image).
	//
