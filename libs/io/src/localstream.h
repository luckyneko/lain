...
// Callers reach these through io::openStream / io::createStream;
...
namespace lain::io
{
	...
	[[nodiscard]] std::unique_ptr<ReadStream> openLocalReadStream(const std::filesystem::path& path, lain::core::Uri uri);
	...
	[[nodiscard]] std::unique_ptr<WriteStream> createLocalWriteStream(const std::filesystem::path& path, lain::core::Uri uri);
} // namespace lain::io
